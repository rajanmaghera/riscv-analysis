  ...
  outputs = { self, nixpkgs, fenix, naersk }:
    let
      ...
      eachCrossSystem = supportedSystems: callback:
        eachSystem supportedSystems (buildSystem:
          builtins.foldl' (inner: targetSystem:
            inner // {
      ...
      truePackages = (eachCrossSystem (builtins.attrNames buildTargets)
        (buildSystem: targetSystem:
          let
            ...
            buildPackageAttrs = if builtins.hasAttr "makeBuildPackageAttrs"
            buildTargets.${targetSystem} then
          ...
          in naersk-lib.buildPackage (buildPackageAttrs // rec {
            src = ./.;
            ...
            CARGO_BUILD_RUSTFLAGS =
              [ "-C" "target-feature=+crt-static" "-C" "linker=${TARGET_CC}" ];
          })));
    in {
      packages = builtins.listToAttrs (builtins.map (buildSystem: {
        name = buildSystem;
        value = let version = truePackages.${buildSystem}.default.version;
        in {
          "my-file" =
            nixpkgs.legacyPackages.${buildSystem}.runCommand "my-file" { } ''
              ...
              cp ${
                truePackages."${buildSystem}".cross-aarch64-linux
              }/bin/rva $out/RVA/Linux-aarch64/${version}/rva
              cp ${
                truePackages."${buildSystem}".cross-x86_64-linux
              }/bin/rva $out/RVA/Linux-x86_64/${version}/rva
              cp ${
                truePackages."${buildSystem}".cross-aarch64-darwin
              }/bin/rva $out/RVA/Darwin-aarch64/${version}/rva
              cp ${
                truePackages."${buildSystem}".cross-x86_64-darwin
              }/bin/rva $out/RVA/Darwin-x86_64/${version}/rva
